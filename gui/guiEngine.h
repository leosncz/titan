...
public:
	...
	void update(scene* scene_)
	{
		...
		if (isVisible)
		{
			...
			{
				showRenderingDebug(scene_->getExposure(),scene_->getGamma(),scene_->getGPosition(), scene_->getGNormals(), scene_->getGAlbedo(), scene_->getGRoughness(), scene_->getGMetallic(),scene_->getGAmbient(),scene_->getSSAOTexture());
			}
