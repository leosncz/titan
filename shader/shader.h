...
public:
    ...
    void compileDefaultShader() //For now handles directionnal and point lights
    {
        ...
        "  vec3 reflectDir = reflect(-lightDir, norm);"
        "  float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);"
        "  vec3 specular = specularStrength * spec * lightColor3;"
        "  vec3 result = ((ambiant*=attenuation) + (diffuse*=attenuation) + (specular*=attenuation)) * final_color;"
        "  return(vec4(result,1.0));"
        "}"
        //""
        ...
        "  vec3 reflectDir = reflect(-lightDir, norm);"
        "  float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);"
        "  vec3 specular = specularStrength * spec * lightColor4;"
        "  vec3 result = ((ambiant*=attenuation) + (diffuse*=attenuation) + (specular*=attenuation)) * final_color;"
        "  return(vec4(result,1.0));"
        "}"
        //""
        ...
        "  vec3 reflectDir = reflect(-lightDir, norm);"
        "  float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);"
        "  vec3 specular = specularStrength * spec * lightColor5;"
        "  vec3 result = ((ambiant*=attenuation) + (diffuse*=attenuation) + (specular*=attenuation)) * final_color;"
        "  return(vec4(result,1.0));"
        "}"
        //""
        ...
        "  vec3 reflectDir = reflect(-lightDir, norm);"
        "  float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32);"
        "  vec3 specular = specularStrength * spec * lightColor6;"
        "  vec3 result = ((ambiant*=attenuation) + (diffuse*=attenuation) + (specular*=attenuation)) * final_color;"
        "  return(vec4(result,1.0));"
        "}"
        //""
